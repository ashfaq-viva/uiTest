...
export function resizeImageIfNeeded(imagePath, expectedWidth) {
  ...
  console.log(`📏 ${path.basename(imagePath)} width: ${png.width}px → expected: ${expectedWidth}px`);

  if (png.width === expectedWidth) {
    console.log(`✅ Width already matches. No resize needed.`);
    return false;
  }
  ...
  fs.writeFileSync(imagePath, PNG.sync.write(resized));
  console.log(`🔧 Resized ${path.basename(imagePath)} to width ${expectedWidth}px`);
  return true;
}
